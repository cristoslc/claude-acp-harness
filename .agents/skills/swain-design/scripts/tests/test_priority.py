...
from specgraph.priority import resolve_vision_weight
...
class TestVisionWeightResolution:
    ...
    def test_vision_default_medium(self):
        assert (
            resolve_vision_weight("VISION-002", self.NODES, self.EDGES) == 2
        )  # medium default

    def test_initiative_inherits_vision_weight(self):
        assert (
            resolve_vision_weight("INITIATIVE-001", self.NODES, self.EDGES) == 3
        )  # inherits high

    def test_initiative_override(self):
        assert (
            resolve_vision_weight("INITIATIVE-002", self.NODES, self.EDGES) == 1
        )  # overrides to low

    def test_epic_inherits_through_initiative(self):
        assert (
            resolve_vision_weight("EPIC-001", self.NODES, self.EDGES) == 3
        )  # EPIC→INIT→VISION(high)

    def test_spec_inherits_through_chain(self):
        assert (
            resolve_vision_weight("SPEC-001", self.NODES, self.EDGES) == 3
        )  # SPEC→EPIC→INIT→VISION(high)

    def test_orphan_returns_default(self):
        assert (
            resolve_vision_weight("ORPHAN-001", self.NODES, self.EDGES) == 2
        )  # medium default
