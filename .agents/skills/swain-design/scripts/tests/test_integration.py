...
class TestNeighbors:
    ...
    def test_neighbors_returns_tsv_rows(self):
        ...
        for line in result.strip().split("\n"):
            ...
            assert (
                len(parts) >= 3
            ), f"Expected at least 3 TSV columns, got {len(parts)}: {line!r}"
...
class TestScope:
    ...
    def test_scope_parent_chain_includes_epic(self):
        ...
        assert (
            "EPIC-013" in result
        ), "SPEC-031 scope should include EPIC-013 in parent chain"
    ...
    def test_scope_siblings_includes_other_specs_under_same_epic(self):
        ...
        sibling_section = result.split("=== Siblings ===", 1)[-1].split(
            "=== Laterals ==="
        )[0]
    ...
    def test_scope_returns_empty_for_unknown_id(self):
        ...
        assert (
            result.strip() == ""
        ), f"Expected empty output for unknown ID, got: {result!r}"
...
class TestImpact:
    ...
    def test_impact_has_required_sections(self):
        ...
        assert re.search(
            r"^DIRECT: \d+", result, re.MULTILINE
        ), "impact output should have 'DIRECT: N' line"
        assert re.search(
            r"^AFFECTED CHAINS: \d+", result, re.MULTILINE
        ), "impact output should have 'AFFECTED CHAINS: N' line"
        assert re.search(
            r"^TOTAL: \d+", result, re.MULTILINE
    ...
    def test_impact_total_is_sum_of_direct_and_chains(self):
        ...
        assert (
            m_direct and m_chains and m_total
        ), "Expected DIRECT/AFFECTED CHAINS/TOTAL lines in output"
...
class TestEdges:
    ...
    def test_edges_tsv_format_three_columns(self):
        ...
        for line in result.strip().split("\n"):
            ...
            assert (
                len(parts) == 3
            ), f"Expected 3 TSV columns, got {len(parts)}: {line!r}"
    ...
    def test_edges_no_filter_returns_all_edges(self):
        ...
        assert (
            len(lines) > 10
        ), f"Global edges should return many edges, got {len(lines)}"
...
class TestNext:
    ...
    def test_next_has_ready_section(self):
        ...
        assert result.strip().startswith(
            "READY:"
        ), "next output should start with 'READY:'"
    ...
    def test_next_ready_section_non_empty(self):
        ...
        artifact_lines = [
            l
            for l in ready_section.split("\n")
            if l.strip() and re.search(r"[A-Z]+-\d+", l)
        ]
        assert (
            len(artifact_lines) > 0
        ), "READY section should have at least one artifact"
    ...
    def test_next_would_unblock_annotation(self):
        ...
        if "SPIKE-018" in result and "would unblock" in result:
            assert (
                "EPIC-014" in result.split("would unblock")[1].split("\n")[0]
...
class TestMermaid:
    ...
    def test_mermaid_node_format(self):
        ...
        assert len(matches) > 0, 'mermaid should contain ID["Title"] nodes'
...
class TestStatus:
    ...
    def test_status_hidden_count_shown(self):
        ...
        result = run_specgraph(["status"])
        assert (
            "resolved artifact" in result or "hidden" in result
    ...
    def test_status_all_flag_shows_more(self):
        ...
        default_count = len(
            [l for l in result_default.split("\n") if re.match(r"\s+[A-Z]+-\d+", l)]
        )
        all_count = len(
            [l for l in result_all.split("\n") if re.match(r"\s+[A-Z]+-\d+", l)]
        )
        assert (
...
class TestOverview:
    ...
    def test_overview_summary_has_counts(self):
        ...
        assert (
            "Ready:" in summary and "Blocked:" in summary
    ...
    def test_overview_unblocked_artifact_has_no_blocked_by(self):
        ...
        if "EPIC-005" in result:
            epic005_line = next((l for l in result.split("\n") if "EPIC-005" in l), "")
            assert (
                "blocked by" not in epic005_line
